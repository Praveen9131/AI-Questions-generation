...
def generate_mcq_with_image_options(subject: str, tone: str):
    ...
    description_prompt = [
        ...
        {"role": "user", "content": f"Generate a clear and understandable multiple-choice question with multiple correct answers based on the subject '{subject}'. Each option should be related to the concept in the subject and in a '{tone}' tone. Ensure the correct answers are included among the options and avoid including explanations. Provide a detailed situation followed by the question. Use the following format:\n\n**Question:** [Detailed situation explaining the context, followed by the question]\n\n**Options:**\n1. [Option 1]\n2. [Option 2]\n3. [Option 3]\n4. [Option 4]\n\n**Correct Answers:** [Correct Options]\n\nEnsure that all four options are provided without explanations and the correct answers are among them."}
    ]
    ...
    try:
        while True:
            ...
            correct_answers_text = content.split("**Correct Answers:**")[1].strip()  # Extract only the answer text

            # Parse options
            options = [option.split('. ')[1] for option in options_section.split('\n') if option.strip()]
            correct_answers = [ans.strip() for ans in correct_answers_text.split(',')]
            if len(options) != 4:
            ...
            # Check if the correct answers are in the list of options
            if not all(ans in options for ans in correct_answers):
                continue  # Retry if the correct answers are not in the list of generated options
            ...
            correct_answers_indices = [options.index(ans) for ans in correct_answers]
            ...
            correct_answers_shuffled_indices = [shuffled_options.index(ans) for ans in correct_answers]
            ...
            return {
                ...
                "correct_answers": [f"option{index + 1}" for index in correct_answers_shuffled_indices]
            }
    except Exception as e:
...
def serve_image(filename):
    ...
    if filename in image_store_checkbox:
        image_data = BytesIO(image_store_checkbox[filename])
        image_data.seek(0)
