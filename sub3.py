...
def download_and_resize_image(image_url, target_size, retries=3):
    ...
    try:
        ...
        output.seek(0)
        image_key = f"image_{len(image_store_sub3) + 1}.png"
        image_store_sub3[image_key] = output
        return image_key
...
def generate_mcq(subject: str, tone: str):
    """Generate a multiple-choice question with a single correct answer based on the subject, with image options."""
    description_prompt = [
        {"role": "system", "content": "You are an expert in generating educational content."},
        {"role": "user", "content": f"Generate a clear and understandable question based on the subject '{subject}'. The question must have exactly one correct answer. Each option should be related to the concept in the subject and in a '{tone}' tone. Use the following format:\n\n**Question:** [Question based on the subject]\n\n**Options:**\n1. [Option 1 Description]\n2. [Option 2 Description]\n3. [Option 3 Description]\n4. [Option 4 Description]\n\n**Correct Answer:** [Correct Option number]\n\nEnsure that all four options are provided, and the correct answer should be just a number (1, 2, 3, or 4). The options should be descriptions of images to generate."}
    ]
    ...
    try:
        ...
        for desc in option_descriptions:
            ...
            image_key = download_and_resize_image(image_url, (750, 319))
            if not image_key:
                raise ValueError("Failed to download and resize image for option")
...
def generate_custom_content_sub3(number, subject, tone):
    ...
    for idx, question in enumerate(questions):
        ...
        for sub_idx, sub_question in enumerate(question["sub_questions"]):
            ...
            if content_type == 'text':
                ...
                image_key = download_and_resize_image(image_url, (750, 319)) if image_url else "placeholder_image_url"
                sub_question_formatted.append({
                    f"Sub Question No. {sub_idx + 1}": {
...
def custom_content():
    ...
    try:
        ...
        result = generate_custom_content_sub3(number=num_questions, subject=subject, tone=tone)
...
def get_image(image_key):
    ...
    try:
        image = image_store_sub3.get(image_key)
        if not image:
